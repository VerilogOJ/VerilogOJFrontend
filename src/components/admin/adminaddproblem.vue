<template>
  <el-form ref="addproblemform" :model="addproblemform" label-position="right">
    ...
    <el-form-item label="题目描述：">
      <el-input
        type="textarea"
        v-model="addproblemform.description"
        autosize
        style="width: 800px"
      ></el-input>
    </el-form-item>
    <el-form-item label="输入描述：">
      <el-input
        type="textarea"
        v-model="addproblemform.description_input"
        autosize
        style="width: 800px"
      ></el-input>
    </el-form-item>
    <el-form-item label="输出描述：">
      <el-input
        type="textarea"
        v-model="addproblemform.description_output"
        autosize
        style="width: 800px"
      ></el-input>
    </el-form-item>
    ...
    <el-form-item>
      <el-upload
        style="width: 400px"
        ref="upload_description"
        action=""
        :http-request="upload"
        :on-success="onSuccessForDescription"
        :on-error="onError"
    ...
    <el-form-item>
      <el-upload
        ...
        action=""
        :http-request="upload"
        :on-success="onSuccessForTemplateCode"
        :on-error="onError"
        :before-remove="beforeRemove"
        :limit="1"
        :file-list="templateCodeFileList"
      >
        <el-button slot="trigger" size="small" type="primary"
          >上传模板代码文件</el-button
        >
    ...
    <el-form-item>
      ...
      >
        ...
        <div slot="tip" class="el-upload__tip">
          包括评测脚本、标准解答、testbench等，具体格式还有待讨论
        </div>
      </el-upload>
    </el-form-item>
...
export default {
  ...
  methods: {
    ...
    beforeRemove(file, fileList) {
      this.$axios
        .delete("/api/files/" + file.response.id + "/")
        .then((response) => {
    ...
    addProblem() {
      ...
      this.$confirm("确定添加吗？", "添加题目", {
        ...
        if (this.haveddl) this.addproblemform.deadline_time = this.ddl;
        ...
        this.$axios
          .post("/problems/", this.addproblemform)
          .then((response) => {
            this.$message.success("题目添加成功！");
          })
          .catch((error) => {
            this.$message.error(
              "题目添加失败：" + JSON.stringify(error.response.data)
            );
          });
      });
    },
